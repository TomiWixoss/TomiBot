...
export function buildStyledCodeBlock(
  ...
  slide.addShape('rect', {
    ...
    shadow: style === 'terminal' ? { type: 'outer', blur: 3, offset: 2, angle: 45, color: '000000', opacity: 0.3 } : undefined,
  });
  ...
  if (style === 'terminal') {
    ...
    slide.addShape('ellipse', { x: typeof x === 'number' ? x + 0.2 : x, y: buttonY, w: 0.15, h: 0.15, fill: { color: 'FF5F56' } });
    slide.addShape('ellipse', { x: typeof x === 'number' ? x + 0.4 : x, y: buttonY, w: 0.15, h: 0.15, fill: { color: 'FFBD2E' } });
    slide.addShape('ellipse', { x: typeof x === 'number' ? x + 0.6 : x, y: buttonY, w: 0.15, h: 0.15, fill: { color: '27CA40' } });
  }
  ...
  // Code text
  const codeY = style === 'terminal'
    ? (typeof y === 'number' ? y + 0.4 : y)
    : (typeof y === 'number' ? y + 0.15 : y);
  const codeHeight = style === 'terminal'
    ? (typeof height === 'number' ? height - 0.5 : height)
    : (typeof height === 'number' ? height - 0.3 : height);
...
export function buildInlineCode(
  text: string,
  theme: PresentationTheme
): any {
  return {
...
export function highlightCode(code: string, language: string): Array<{ text: string; color?: string }> {
  ...
  const keywords: Record<string, string[]> = {
    ...
    python: ['def', 'class', 'if', 'else', 'elif', 'for', 'while', 'return', 'import', 'from', 'as', 'try', 'except', 'with', 'lambda', 'async', 'await'],
